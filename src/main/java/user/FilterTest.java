		...
		if(session != null) {
			try {
				...
				if(lang.equals("all") && topic.equals("all") && level.equals("all")) {
					if(person == "user") response.sendRedirect("/mind_gauge/UserDashboard");
					if(person == "admin") response.sendRedirect("/mind_gauge/AdminDashboard");
				}
			...
			response.sendRedirect("/mind_gauge/error.jsp");
		}
	}
